# Sara Dunlop (Developer website and portfolio)

## Introduction

This is my official developer website and portfolio. It is a constant work in progress. 

### Table of Contents
1. Description
2. Updates
3. Coming Soon
4. Build
5. Links

## Description
This is my official developer website. It includes:
- My developer skills/range of knowledge
- A portfolio of some of my favorite projects
- A blog, which contains (or will contain) posts about my projects, my programming journey, things I'm learning, and programming-based opinion pieces. I may also begin posting about my career search soon.
  
## Updates
### Prior Recent Updates
The following are features I recently implemented before the inclusion of this Updates section.

- **Restructured website files**: I restructured my entire website in a way that makes more sense. This included things like adding a "Main Pages" folder (which includes all of the main pages including [the homepage](https://risclover.github.io), [my portfolio](https://risclover.github.io/main%20pages/portfolio), [my blog](https://risclover.github.io/main%20pages/blog.html), and, in the future, my contact page), a "Legal Pages" folder, and individual folders for my JS files and my stylesheets.

    I completely redid the "Blog" section of my files, too, and within the "Blog" folder, I created individual folders for Posts, Images, Categories, Archives, and Tags.
- Added legal pages:
  - [Privacy Policy](https://risclover.github.io/legal%20pages/privacypolicy.html)
  - [Terms and Conditions](https://risclover.github.io/legal%20pages/termsandconditions.html)
  - [Cookie policy](https://risclover.github.io/legal%20pages/cookiepolicy.html)
  - [DMCA policy](https://risclover.github.io/legal%20pages/dmcapolicy.html)
  - [Disclaimer](https://risclover.github.io/legal%20pages/disclaimer.html)
- (*Finally*) added a footer, which includes:
  - A copyright stamp
  - A link to my github
  - Links to the legal pages
  - Link to a sitemap (coming soon)
- Added an icon-based button on all pages that, when clicked, brings you to the top. After scrolling down a little bit, this button appears on the bottom right part of each page (as a "floater" button).

## Coming Soon
These are updates and features that are coming up next in my list of things to update.

- **Fix footer links**: I noticed today that the links to the legal pages (which sit in my footer) are all broken because the file path is incorrect. 
- **Change footer on all pages**: My homepage currently has a different footer than the other pages. This is the footer I will be implementing for  all pages. It will contain a link to a sitemap, the Privacy Policy, the Terms and Conditions, and a page containing links to the other "legal pages".
- **Create sitemap**: I don't have the aforementioned sitemap created yet. This is high up on my priorities list.
- **New portfolio categories**: On my Portfolio page, I'm going to implement an additional row of categories. It currently contains programming language-based categories (HTML, CSS, JavaScript, jQuery, etc). I'm also going to have utility-based categories so you can filter my projects by things like "Games", "Tools", etc.
- **Make a decision about my blog**: I am currently building my entire blog 100% from scratch without a CMS; just plain old HTML, CSS, and JavaScript (plus a framework or two). The more pages I add to my blog, the more difficult this becomes, because, I, genius that I am, decided to create unique pages for every single category and tag, which is on top of the pages for each blog post, the Archives page, etc. It's just a lot. So I need to decide whether I'm willing to continue this (because I do love my blog), or whether I'm going to try to transfer everything to an already-established blog program. I'm considering Wix because I already know how to use it and it contains everything I want in a blog, but Wordpress isn't off the table.

## Build
This website was built with the following languages:
* HTML
* CSS
* JavaScript
* jQuery
* Bootstrap

This list is likely to change as I learn new languages, libraries, and frameworks.

## Links

- [Live demo](https://risclover.github.io)
- [Code repo](https://www.github.com/Risclover/risclover.github.io)